...
interface ClientToServerEvents {
  ...
  "provider-accept-order": (data: {
    orderId: number;
    customerUuid: string;
  }) => void;
  "provider-reject-order": (data: {
    orderId: number;
    customerUuid: string;
  }) => void;
...
const addOrderHistory = async (orderId: number, reason: OrderHistory) => {
  console.log(
    "[SOCKET - Add order history] Add order history, Order ID : " +
      orderId +
      " Reason : " +
      reason.toString()
  );
...
const broadcastOnlineProvider = (socket: CustomSocket) => {
  const filteredProviders = onlineProviders.filter(
    (provider) => provider.status === ProviderStatus.Online
  );
  console.log(
...
const addOnlineProvider = (
  ...
  console.log(
    "[SOCKET - Add/Update providers] Updating online providers, Provider ID : " +
      id
  );
  console.log("[SOCKET - Add/Update providers] Received new statuss " + status);
  const filteredOnlineProviders = onlineProviders.filter(
  ...
  console.log(
    "[SOCKET - Add/Update providers] New online providers : ",
    onlineProviders
  );
...
const removeOnlineProvider = (id: number, socket: CustomSocket) => {
  console.log(
    "[SOCKET - Remove online provider] Provider went offline, Provider ID : " +
      id
  );
...
const addPendingOrder = (
  ...
  console.log(
    "[SOCKET - Add new order] Adding new order. OrderID : " +
      orderId +
      " ProviderUuid : " +
      providerUuid
  );
...
const removePendingOrder = async (
  ...
  if (order) {
    if (!ignoreArrayRemove)
      activeOrders = activeOrders.filter((order) => order.orderId !== orderId);
...
const getOnlineProvider = (id: any, searchKey: keyof ProviderSocket = "id") => {
  const provider = onlineProviders.find(
    (provider) => provider[searchKey] === id
  ) as ProviderSocket;
  return provider;
};
...
const getActiveOrders = (
  id: any,
  searchKey: keyof ActiveOrders = "orderId"
) => {
  const order = activeOrders.find(
    (order) => order[searchKey] === id
  ) as ActiveOrders;
  return order;
};

const setProviderOffline = (providerId: number, socket: CustomSocket) => {
  const providerOrders = activeOrders.filter(
    (order) => order.providerUuid === socket.id
  );
...
io.on("connection", (socket) => {
  ...
  socket.on("provider-online-location-change", (args) => {
    ...
    if (order) {
      ...
      socket
        ...
        .emit("provider-to-customer-location-change", {
          latitude: args.latitude,
          longitude: args.longitude,
  ...
  socket.on("provider-accept-order", async (args) => {
    ...
    if (order) {
      ...
      addOnlineProvider(
        { ...provider, status: ProviderStatus.HaveOrder },
        socket,
        true
      );
  ...
  socket.on("provider-reject-order", async (args) => {
    ...
    socket
      ...
      .emit("order-rejected", { result: false, orderId: args.orderId });
  });
